...
const renderAllKey: InjectionKey<boolean> = Symbol('osw:tabsRenderAll')
...
export function useTabsSelectedState(
  ...
  if (!sharedState) {
    throw new Error('[osw-tabs] TabsSharedState should be injected')
  }
...
export function useTabsSingleState() {
  ...
  if (!state) {
    throw new Error('[osw-tabs] TabsSingleState should be injected')
  }
  return state
}
